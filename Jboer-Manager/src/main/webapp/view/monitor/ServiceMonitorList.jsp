...
function editMonitor(serviceNodeId){
	if(serviceNodeId != ""){ 
		...
		$.post('<%=path%>/monitor/serviceMonitor/editServiceMonitor.do?serviceId=' + serviceNodeId,
					function(result) {
						if (result.success) {
...
function deleteMonitor(serviceNodeId){
	if(serviceNodeId != ""){ 
		$.messager.confirm("系统提示","您确定要删除这条记录吗?",function(r){
			if(r){
				$.post('<%=path%>/monitor/serviceMonitor/deleteServiceMonitor.do?serviceId='+serviceNodeId,
				function(result){
					if(result.errorMsg){
						$.messager.alert("系统提示",result.errorMsg);
						return;
...
function saveMonitor(){
	$('#fm').form('submit',{
		url:"<%=path%>/monitor/serviceMonitor/saveService.do",
		onSubmit:function(){
			...
			var bol=false;
				$.ajax({
		            type: "post",
		            url: "<%=path%>/monitor/serviceMonitor/searchRepeatService.do",
...
<body>
	...
	<div class="biaodan-content-nav" onclick="openOrOff(this)"><span class="nav-span-left" >服务监控查询区</span></div>
	<div class="biaodan-content-List">
		<table style="width: 100%; margin-bottom: 10px;">
			<tr>
				<td style="font-size: 12px; width: 30%; text-align: center;">
					<span style="height: 30px; vertical-align: middle; font-weight: bold; width: 40%; text-align: right">服务名称:</span>
					<input id="service_name" name="service_name" style="width: 50%;" />
				</td>
				<td></td>
			    <td></td>
			</tr>
			<tr>
			 <td style="font-size: 12px;width: 70%;text-align:center;" colspan="3">
				<a href="javascript:void(0)" class="standard-button" onclick="queryMonitorList()">查询</a>
	   ...
	   <table style="width: 100%;margin-bottom:10px;">
		<tr>
			<td style="font-size: 12px;width: 70%;text-align:center;">
				<a href="javascript:void(0)" style="float:right;margin-right:10px;" class="standard-button" onclick="newMonitor()">新增监控服务</a>
			</td>
		</tr>
	</table>
			...
			<form id="fm" method="post">
				...
				<table cellspacing="10px;">
					<tr>
						<td style="font-size: 12px; width: 30%; text-align: center;">
						<span style="height: 30px; vertical-align: middle; font-weight: bold; width: 40%; text-align: right">服务名称:</span>
						<input id="servname_id" name="serviceName" data-options="required:true" maxlength="50" style="width: 50%;"/>
						</td>
					</tr>
					<tr>
					<td style="font-size: 12px; width: 30%; text-align: center;">
						<span style="height: 30px; vertical-align: middle; font-weight: bold; width: 40%; text-align: right">服务地址:</span>
						<input id="servurl_id" name="serviceUrl" data-options="required:true" maxlength="200" style="width: 50%;"/>
						</td>
					</tr>
				</table>
			</form>
